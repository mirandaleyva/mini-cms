...
{
  private array $cache = []; // Alle Cache-einträge speichern

  public function get(string $key): mixed // Gibt den wert für den angegebenen Schlüssel zurück
  {
    return $this->cache[$key] ?? null; // Wenn der Schlüssel nicht existiert, wird null zurückgegeben
  }

  public function set(string $key, mixed $value): void // Speichert einen Wert unter dem angegebenen Schlüssel
  {
    $this->cache[$key] = $value; // Wert im internen Array speichern
  }

  public function has(string $key): bool // Prüft ob ein Wert für den angegebenen Schlüssel existiert
  {
    return array_key_exists($key, $this->cache); // Gibt true zurück, wenn der Schlüssel existiert, sonst false
  }
}